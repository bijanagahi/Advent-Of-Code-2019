...
class Line(object):
	...
	def intersects(self, line):
		return (line.start < self.cost < line.end) and (self.start < line.cost < self.end)
...
def main():
	wire1x, dir1 = parseWire(wire1)
	wire2x, dir2 = parseWire(wire2)
	print(dir1)
	print(dir2)
	# make the first set of lines
	curPos = (0,0)
	wire1LinesH = []
	wire1LinesV = [] 
	wire2LinesH = []
	wire2LinesV = [] 
	#nodes[(0,0)] = -1
	for k in range(0,len(wire1x),2): # lineDist in wire1:
		# Change first to H/V based on input
		wire1LinesH.append(Line(curPos[0],curPos[0] + wire1x[k], curPos[1]))
		curPos = (curPos[0]+wire1x[k], curPos[1])
		if (k+1 < len(wire1x)):
			wire1LinesV.append(Line(curPos[1],curPos[1] + wire1x[k+1], curPos[0]))
			curPos = (curPos[0], curPos[1]+wire1x[k+1])
	
	curPos = (0,0)
	for k in range(0,len(wire2x),2): # lineDist in wire1:
		# Change first to H/V based on input
		# wire2LinesV.append(Line(curPos[1],curPos[1] + wire2x[k], curPos[0]))
		# curPos = (curPos[0], curPos[1]+wire2x[k])
		# if (k+1 < len(wire2x)):
		# 	wire2LinesH.append(Line(curPos[0],curPos[0] + wire2x[k+1], curPos[1]))
		# 	curPos = (curPos[0]+wire2x[k+1], curPos[1])
		wire2LinesH.append(Line(curPos[0],curPos[0] + wire2x[k], curPos[1]))
		curPos = (curPos[0]+wire2x[k], curPos[1])
		if (k+1 < len(wire2x)):
			wire2LinesV.append(Line(curPos[1],curPos[1] + wire2x[k+1], curPos[0]))
			curPos = (curPos[0], curPos[1]+wire2x[k+1])
		
	# print('|'.join(str(_) for _ in wire1LinesH)	)	
	# print('|'.join(str(_) for _ in wire1LinesV)	)	
	# print('|'.join(str(_) for _ in wire2LinesH)	)	
	# print('|'.join(str(_) for _ in wire2LinesV)	)	

	ans = 0
	for k in wire1LinesH:
		for j in wire2LinesV:
			if k.intersects(j):
				dist = abs(k.cost) + abs(j.cost)
				print(k,j,dist)
				if (dist < ans or ans == 0):
					ans = dist

	for k in wire1LinesV:
		for j in wire2LinesH:
			if k.intersects(j):
				dist = abs(k.cost) + abs(j.cost)
				print(k,j,dist)
				if (dist < ans or ans == 0):
